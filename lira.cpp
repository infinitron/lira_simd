...
class Ops
{
  public:
    ...
    template<class T, class Tv, class tagF>
    static void update_alpha_ms(AsyncParamFileIO<T, Tv, tagF>& t_out_file, MultiScaleMap<T, Tv, tagF>& t_ms);
...
PSF<uPtr_F, tagF>::initialize()
{
    ...
    if (is_psf_prag_bayesian) { //rescale the original PSF with its min value and reverse it
        ...
        //get the minimum non-zero pixel-value
        pix_type val = -1;
        for (size_t i = 0; i < m_npixels - 1; ++i) {
            auto cur = m_mat.get()[i];
            auto next = m_mat.get()[i + 1];

            if (cur < next && cur != 0)
                val = cur;
            else if (cur != next)
                val = next;
            //else no update. Both the values are zero
        }
        m_min_psf = val;
        if (val == -1) {
...
PSF<uPtr_F, tagF>::get_rmat()
{
    if (is_psf_prag_bayesian && m_config->iter % Constants::N_ITER_PER_PSF == 0) {
        ...
        //create a new PSF from the original scaled PSF
        std::transform(Constants::execParUnseq, m_mat.get(), m_mat.get() + m_npixels, m_rmat.get(), [&](const auto i) {
            return rpois(i) * m_min_psf;
        });

        //re-normalize
        auto sum = std::reduce(m_rmat.get(), m_rmat.get() + m_npixels, 0.0, std::plus<pix_type>());
        std::transform(m_rmat.get(), m_rmat.get() + m_npixels, m_rmat.get(), [&](const auto& a) { return a / sum; });
    }
    return m_rmat;
}
...
CountsMap<uPtr_F, uPtr_Fv>::re_norm_img(typename CountsMap<uPtr_F, uPtr_Fv>::pix_type t_val)
{
    ...
    if (i < m_npixels) { //the should should not show up here
...
template<class T, class Tv, class tagF>
void
Ops::update_alpha_ms(AsyncParamFileIO<T, Tv, tagF>& t_param_file, MultiScaleMap<T, Tv, tagF>& t_ms)
{
    double lower, //lower
      middle,     //middle
      upper,      //upper
...
//g++ -shared lira.cpp -lgtest -lpthread -lR -lhwy -I/usr/local/include/ -I/usr/share/R/include/ -O2 -fPIC -o lira2.so -O2 -std=c++17 -ltbb
